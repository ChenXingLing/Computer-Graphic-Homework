...
public:
	...
	MyImage(BYTE* data, int ww, int hh, int ncomp,int awa,int qwq) :w(ww), h(hh), comp(ncomp){
		pixels.resize(w*h*comp);
		std::copy_n(data, w*h*comp, pixels.data());
	}
	inline void reverse(){
		std::vector<BYTE> tmp=pixels;
		for(int i=0;i<h;++i)
			for(int j=0;j<w;++j)
				for(int k=0;k<comp;++k)
					pixels[(j*h+i)*comp+k]=tmp[(i*w+j)*comp+k];
		std::swap(h,w);
	}
